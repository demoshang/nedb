...
const modifierFunctions = {
  ...
  $setOnInsert: createModifierFunction((obj, field, value) => {
    obj[field] = value
  }),
  /**
   * Unset a field
  ...
  $mul: createModifierFunction((obj, field, value) => {
    if (typeof value !== 'number') throw new Error(`${value} must be a number`)

    if (typeof obj[field] !== 'number') {
      throw new Error('Don\'t use the $mul modifier on non-number fields')
    } else obj[field] *= value
  }),
  /**
 ...
 */
const modify = (obj, updateQuery, upsert) => {
  ...
  if (dollarFirstChars.length === 0) {
    ...
    const index = modifiers.indexOf('$setOnInsert')
    if (index >= 0) {
      // remove $setOnInsert
      modifiers.splice(index, 1);

      // let $setOnInsert firt
      if(upsert) {
        modifiers.unshift('$setOnInsert')
      }
    }
...
const comparisonFunctions = {
  ...
  $eq: (a, b) => a !== undefined && areThingsEqual(a, b),
  $mod: (a, b) => {
    if (!Array.isArray(b)) throw new Error('$mod operator called with a non-array')
    if (b.length !== 2) throw new Error('$mod operator called with 2 item array')
    return a % b[0] === b[1]
  },
  /** Is in Array */
  $in: (a, b) => {
  ...
  /** Matches Regexp */
  $regex: (a, b, options) => {
    let reg = (typeof b === 'string' || isRegExp(b)) ? new RegExp(b, options) : b
    if (!isRegExp(reg)) throw new Error('$regex operator called with non regular expression')

    if (typeof a !== 'string') return false
    else return reg.test(a)
  },
...
function matchQueryPart (obj, queryKey, queryValue, treatObjAsValue) {
  ...
  if (queryValue !== null && typeof queryValue === 'object' && !isRegExp(queryValue) && !Array.isArray(queryValue)) {
    ...
    // 删除 $options 参数, 只有在 $regex 时自动使用
    const index = keys.indexOf('$options')
    if (index >= 0) {
      keys.splice(index, 1);
    }
    ...
    if (dollarFirstChars.length > 0) {
      for (const key of keys) {
        ...
        if(key === '$regex') {
          if (!comparisonFunctions[key](objValue, queryValue[key], queryValue['$options'])) return false
